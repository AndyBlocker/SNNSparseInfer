                      ...
                      int M, int K, int N, int tile)
{
    ...
    static cudaEvent_t stage1 = nullptr, stage2 = nullptr, stage3 = nullptr, stage4 = nullptr, stage5 = nullptr;
    if (!beg) { 
        cudaEventCreate(&beg); 
        cudaEventCreate(&end);
        ...
        cudaEventCreate(&stage4);
        cudaEventCreate(&stage5);
    }
    ...
    cudaEventRecord(stage5);

    /* --------------- 6. 计时 + 返回 -------------------------- */
    cudaEventRecord(end);
    cudaEventSynchronize(end);
    
    printf("Stage 0 (buildMask): %.3f ms\n", to_ms(beg, stage1));
    printf("Stage 1 (reduceColumnMask + prefixSum): %.3f ms\n", to_ms(stage1, stage2));
    printf("Stage 2 (gatherTiles): %.3f ms\n", to_ms(stage2, stage3));
    printf("Stage 3 (cuBLAS GEMM): %.3f ms\n", to_ms(stage3, stage4));
    printf("Stage 4 (scatterTiles): %.3f ms\n", to_ms(stage4, stage5));
    
    return to_ms(beg, end);
}

/* ===================================================================== *
 *                             小测试 (可选)                            *
 * ===================================================================== *
 * 如果想在单文件里做 quick‑check，可以在 main() 里创建小矩阵 call 上述  *
 * runBasicSparse(...)。此处留空，避免破坏库式用法。                    *
 * --------------------------------------------------------------------- */
// int main() { /* 自行填写 */ }
